...
;============SSEG=============================================================
			.DSEG
			.def Ml		  = r0				;
			.def Mh		  = r1				; Регистр M - регистр заполняемый функцией MUL
...
LCD_display_mode:.byte 1							;Режим отображения экрана
			...
			.equ DM_MENU_EEPROM_CL		= 13		;	- Очистить EEPROM
			.equ DM_MENU_END			= 14		;До какого числа (не включительно) существует меню

EEPROM_mode:.byte 1									;Флаг состония работы с энергонезависимой памятью
			.equ EEM_READY				= 0			;Память готова к работеы
			.equ EEM_CLRH				= 1			;Очищаем заголовок памяти
			.equ EEM_CLRB1				= 2			;Очищаем первую страницу текущего блока памяти
			.equ EEM_READH				= 3			;Читаем шапку
			.equ EEM_READP1				= 4			;Читаем первую страницу
			.equ EEM_SREADH				= 5			;Читаем шапку чтобы узнать куда там сохранять потом данные
			.equ EEM_SETP1				= 6			;Сохраняем данные
			.equ EEM_SETH				= 7			;Сохраняем новый заголовок в несколько шагов. Это самое начал
			.equ EEM_RSETH				= 8			;Читаем заголовок, чтобы его обновить
			.equ EEM_UPDATEH			= 10		;И вот мы отправили заголовок в EEPROM
...
;----------------------------------------------------------------------------------	
			...
			//Прочитаем данные по последней поездке
			SetTask TS_EEPROM_Read
...
LCD_M_EE_CLR:			.DB "EEPROM CLR",0x00,0x00
